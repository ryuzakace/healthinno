...
class basicView(TemplateView):
    ...
    def get(self, request):
        return HttpResponse("your stairway lies is the whispering winds?")
...
class PatientListView(ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
        valuefrominput = 2
        y = models.Sensor.objects.filter(patient = valuefrominput)
        print(y)
        for i in y:
            print(i)
            models.Data.objects.filter(sensor = i).delete()
        print(request.POST.get('initial_date'))
        p = models.Data.objects.create(value=3, time = 33, sensor_id = 3)
